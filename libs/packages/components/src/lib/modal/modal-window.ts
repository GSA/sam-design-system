...
@Component({
  ...
  host: {
    '[class]': '"modal fade show display-block" + (windowClass ? " " + windowClass : "")',
    'role': 'dialog',
    'tabindex': '-1',
  ...
  template: `
    <div [class]="'modal-dialog' + (size ? ' modal-' + size : '') + (centered ? ' modal-dialog-centered' : '') + (insideScrollbar ? ' modal-dialog-scrollable' : '')" role="document">
...
export class NgbModalWindow implements OnInit,
  ...
  @Input() insideScrollbar: boolean | string = true;
