...
describe('SearchListLayoutComponent', () => {
  ...
  it('should call updateFilterModel', () => {
    let service = fixture.debugElement.injector.get(SDSFormlyUpdateModelService);
    const serviceSpy = spyOn(service, 'updateModel').and.callThrough(); // create spy
    const filterData = {filterModel:{searchKeyword: 'test', entity: 'testEntity'}};
  ...
  it('should set configuration & sortField on ngOnChanges', () => {
    component.ngOnChanges({
      configuration: new SimpleChange(null, {defaultSortValue: 'testValue'}, null)
    });
    fixture.detectChanges();
