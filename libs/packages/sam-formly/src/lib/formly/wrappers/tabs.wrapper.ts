...
@Component({
  template: `
    <label [attr.for]="id" class="usa-label text-bold text-base-dark">{{
      to.label
    }}</label>
    <p [innerHTML]="to.description"></p>
    <div class="sds-filter-keywords">
      <ng-container
        *ngIf="field.fieldArray?.fieldGroup?.length > 1; else singleField"
      >
...
export class FormlyTabsWrapperComponent extends FieldWrapper implements OnInit {
  ...
  ngOnInit() {
    if (!this.field.fieldArray || !this.field.fieldArray.fieldGroup) {
      throw new Error(
        'Please define contents of keywords through a fieldGroup within the fieldArray property'
      );
    }

    this._initialModel =
      this.model && this.model[this.key as string]
        ? { ...this.model[this.key as string] }
        : {};
    if (this.field.fieldArray && this.field.fieldArray.fieldGroup) {
      this.field.fieldArray.fieldGroup.forEach(
