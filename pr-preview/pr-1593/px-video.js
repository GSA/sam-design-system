function InitPxVideo(options) {
  ...
  function launchFullScreen(elem) {
    if (
      !elem.fullscreenElement && // alternative standard method
  ...
  if (
    obj.browserName === "IE" &&
    (obj.browserMajorVersion === 8 || obj.browserMajorVersion === 9)
  ) {
    return false;
  }

  // If smartphone or tablet, stop customization as video (and captions in latest devices) are handled natively
  obj.isSmartphoneOrTablet = /Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(
    navigator.userAgent
  );
  ...
  obj.controls.innerHTML =
    '<div class="clearfix">' +
    '<div class="pull-left">' +
    '<button class="px-video-restart" title="' +
    GLOBAL_STRINGS["RESTART"] +
    '"><span class="sr-only">' +
    GLOBAL_STRINGS["RESTART"] +
    "</span></button>" +
    '<button class="px-video-rewind" title="' +
    GLOBAL_STRINGS["REWIND"] +
    '"><span class="sr-only">rewind <span class="px-seconds">10</span> seconds</span></button>' +
    '<button class="px-video-play" aria-label="' +
    obj.playAriaLabel +
    '" title="' +
    GLOBAL_STRINGS["PLAY"] +
    '"><span class="sr-only">' +
    GLOBAL_STRINGS["PLAY"] +
    "</span></button>" +
    '<button class="px-video-pause hide" title="' +
    GLOBAL_STRINGS["PAUSE"] +
    '"><span class="sr-only">' +
    GLOBAL_STRINGS["PAUSE"] +
    "</span></button>" +
    '<button class="px-video-forward" title="' +
    GLOBAL_STRINGS["FORWARD"] +
    '""><span class="sr-only">forward <span class="px-seconds">10</span> seconds</span></button>' +
    "</div>" +
    '<div class="px-video-mute-btn-container pull-left" title="' +
    GLOBAL_STRINGS["MUTE"] +
    '">' +
    ...
    obj.randomNum +
    '" class="px-video-volume" type="range" min="0" max="10" value="5" />' +
    "</div>" +
    '<div class="px-video-captions-btn-container pull-left hide" title="' +
    GLOBAL_STRINGS["CAPTIONS"] +
    '">' +
    '<input class="px-video-btnCaptions sr-only" id="btnCaptions' +
    obj.randomNum +
    '" type="checkbox" />' +
    '<label for="btnCaptions' +
    obj.randomNum +
    '"><span class="sr-only">' +
    GLOBAL_STRINGS["CAPTIONS"] +
    "</span></label>" +
    "</div>" +
    '<div class="px-video-fullscreen-btn-container pull-left show" title="' +
    GLOBAL_STRINGS["TOGGLE_FULL_SCREEN"] +
    '">' +
    '<input class="px-video-btnFullScreen sr-only" id="btnFullscreen' +
    obj.randomNum +
    '" type="checkbox" />' +
    '<label for="btnFullscreen' +
    obj.randomNum +
    '"><span class="sr-only">' +
    GLOBAL_STRINGS["TOGGLE_FULL_SCREEN"] +
    "</span></label>" +
  ...
  obj.progressBar = obj.container.getElementsByClassName(
    "px-video-progress"
  )[0];
  obj.progressBarSpan = obj.progressBar.getElementsByTagName("span")[0];
  obj.captionsContainer = obj.container.getElementsByClassName(
    "px-video-captions"
  )[0];
  ...
  obj.progressBarHoverContainer = obj.container.getElementsByClassName(
    "px-timetip"
  )[0];
    ...
    if (obj.isTextTracks) {
      ...
      obj.movie.addEventListener(
        ...
        function() {
          ...
          if (
            obj.movie.currentTime.toFixed(1) >
              video_timecode_min(obj.captions[obj.subcount][0]) &&
            obj.movie.currentTime.toFixed(1) <
              video_timecode_max(obj.captions[obj.subcount][0])
          ) {
