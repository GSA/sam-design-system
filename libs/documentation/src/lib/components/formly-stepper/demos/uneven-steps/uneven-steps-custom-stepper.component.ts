...
export class UnevenStepsCustomStepperComponent extends SdsStepper
  ...
  async changeStep(stepId: string, incrementor?: 1 | -1) {
    const startingStep: SdsStepComponent = this.flatSteps.find(
      (step) => step.id === stepId
    );
    ...
    if (mainStepIndex === -1) {
      ...
      if (incrementor === 1) {
        this.stepLabels[mainStepIndex].completionPercent =
          startingStep.fieldConfig.templateOptions.completedValue ?? 0;
      } else {
        // Need to get previous step to know what it's completedValue is.
        const currentFlatStepsIndex = this.flatSteps.findIndex(
          (step) => step.id === this.selectedStep.id
        );
        this.stepLabels[mainStepIndex].completionPercent =
          this.flatSteps[currentFlatStepsIndex - 1].fieldConfig.templateOptions
            .completedValue ?? 0;
      }
      // If main step
    } else {
