...
function main() {
  ...
  const { error: angularJsonError, contents: angularJson } = loadJson(
    angularJsonPath
  );
  ...
  libNames.forEach(lib => {
    ...
    if (lib === 'components') {
      execSync(
        'cp -r ./libs/packages/components/src/assets/ ./dist/libs/components/assets'
      );
    }
    ...
    if (tarballPath) {
      const publishCmd = `npm publish ${tarballPath} ${
...
function findPublishableLibs(angularJson) {
  ...
  const libNames = projectNames.filter(name => {
    ...
    return (
      ...
      metadata.architect.build.builder ===
