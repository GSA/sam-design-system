...
import { sdsPositioning, PlacementArray } from '../util/positioning';
 ...
 */
@Directive({ selector: '[sdsPopover]', exportAs: 'sdsPopover', standalone: true })
export class SdsPopover implements OnInit, OnDestroy, OnChanges {
  ...
  /**
   * |         |closeOnContentClick|closeOnClickOutside|
   * |---------|-------------------|-------------------|
   * |'false'  |false              |false              |
   * |'inside' |true               |false              |
   * |'outside'|false              |true               |
   ...
   */
  @Input() sdsPopover: string | TemplateRef<any> | null | undefined;
   ...
   * If the title and the content are falsy, the popover won't open.
   */
  @Input() sdsPopoverTitle: string | TemplateRef<any> | null | undefined;
   ...
   * By default, the element `sdsPopover` directive is applied to will be set as a target.
   *
   * @since 13.1.0
   */
  ...
  private _sdsPopoverWindowId = `sds-popover-${nextId++}`;
  private _popupService: PopupService<SdsPopoverWindow>;
  private _windowRef: ComponentRef<SdsPopoverWindow> | null = null;
  private _unregisterListenersFn;
  private _positioning: ReturnType<typeof sdsPositioning>;
  private _zoneSubscription: Subscription;
  private _isDisabled(): boolean {
    if (this.disablePopover) {
      return true;
    }
    if (!this.sdsPopover && !this.sdsPopoverTitle) {
      return true;
    }
    return false;
  }
  ...
  constructor(
    ...
    this.position = config.placement;
    ...
    this._popupService = new PopupService<SdsPopoverWindow>(
      SdsPopoverWindow,
      injector,
      viewContainerRef,
      _renderer,
  ...
  open(context?: any) {
    if (!this._windowRef && !this._isDisabled()) {
      ...
      this._windowRef.setInput('title', this.sdsPopoverTitle);
      this._windowRef.setInput('context', context ?? this.popoverContext);
      this._windowRef.setInput('popoverClass', this.popoverClass);
      this._windowRef.setInput('id', this._sdsPopoverWindowId);

      this._renderer.setAttribute(this._getPositionTargetElement(), 'aria-describedby', this._sdsPopoverWindowId);
      ...
      sdsAutoClose(this._ngZone, this._document, this.autoClose, () => this.close(), this.hidden, [
  ...
  ngOnChanges({ sdsPopover, popoverTitle, disablePopover, popoverClass }: SimpleChanges) {
    if (popoverClass && this.isOpen()) {
      this._windowRef!.instance.popoverClass = popoverClass.currentValue;
    }
    // close popover if title and content become empty, or disablePopover set to true
    if ((sdsPopover || popoverTitle || disablePopover) && this._isDisabled()) {
      this.close();
    }
  }
