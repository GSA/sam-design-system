...
const samDependencies = {
  ...
  '@gsa-sam/components': '16.0.12',
  '@gsa-sam/sam-formly': '16.0.12',
  '@gsa-sam/sam-material-extensions': '16.0.12',
};
  ...
  files: [
    {
      ...
      source: fileContent('misc', 'stackblitzes-templates', 'polyfills.ts'),
    },
    {
      name: 'tsconfig.json',
      source: fileContent('misc', 'stackblitzes-templates', 'tsconfig.json'),
    },
    {
      name: 'angular.json',
      source: fileContent('misc', 'stackblitzes-templates', 'angular.json'),
    },
  ],
};
...
modulesInfo.forEach((value, demoModule) => {
  ...
  stackblitzData.files.push({
    name: 'src/index.html',
    source: indexFile(stackblitzData),
  });
  stackblitzData.files.push({
    name: 'src/main.ts',
    source: mainFile(stackblitzData),
  });
  stackblitzData.files.push({
    name: 'src/app.module.ts',
    source: appModule(stackblitzData),
  });
  stackblitzData.files.push({
    name: 'src/app.component.ts',
    source: appComponent(stackblitzData),
  });
  for (const file of demoFiles) {
    ...
    if (fs.existsSync(serviceFolder)) {
      ...
      for (const serviceFile of serviceFiles) {
        ...
        stackblitzData.files.push({
          name: `src/services/${destServiceFile}`,
          source: fs.readFileSync(serviceFile).toString(),
        });
      }
    }
  }
