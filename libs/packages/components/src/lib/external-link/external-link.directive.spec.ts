...
@Component({
  template: `
    ...
    <a
      ...
      [hideIcon]="true"
      href="google.com"
      aria-label="test aria label - opens in a new window"
      >Google
    </a>
    <a id="test3">Not Google </a>
    <a id="test4" href="{{ name }}/settings/test123" aria-label="Test label"
      >Google
    </a>
    <a
      id="test5"
      [hideIcon]="true"
      href="google.com"
      aria-label="test aria label with no keywords"
      >Google
    </a>
    <a id="test6" [hideIcon]="true" href="google.com"
      >Google <span>test element</span></a
    >
...
describe('Sam External Link Directive', () => {
  ...
  it('Should not update aria label to external link if valid aria label is set', () => {
    ...
    expect(testElementWithoutAriaLabel.attributes['aria-label']).toEqual(
      'test aria label - opens in a new window'
    );
  });
  ...
  it('Should update aria label to external link if existing aria label does not indicate opening in new window', () => {
    ...
    expect(
      testElementWithoutAriaLabel.nativeElement.getAttribute('aria-label')
    ).toEqual('test aria label with no keywords - opens in a new window');
  });
  ...
  it('Should treat links that start with mailto as internal links', () => {
    ...
    const externalLinkIcon = mailToLink.nativeElement.querySelector(
      'usa-link--external'
    );
